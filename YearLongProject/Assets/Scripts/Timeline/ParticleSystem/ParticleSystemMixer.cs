...
namespace Timeline.ParticleSystemTimeline
{
    ...
    public class ParticleSystemMixer : PlayableBehaviour
    {
        ...
        private readonly List<Playable> clips = new();
        ...
        public override void ProcessFrame(Playable playable, FrameData info, object playerData)
        {
            ...
            var ps = (ParticleSystem)playerData;
        ...
        private void ProgressivelySimulate(Playable playable, FrameData info, ParticleSystem ps)
        {
            ...
            if (playable.GetTime() < previousClipStart || playable.GetTime() > previousClipEnd || timeLineReset)
            {
                ps.Stop();
                ps.Play();
                timeLineReset = false;
            ...
            if (clipIsPlaying)
            {
                ...
                ParticleSystemBehaviour
                    psb = ((ScriptPlayable<ParticleSystemBehaviour>)currClipPlayable).GetBehaviour();
        ...
        private void HandleScrubbing(Playable playable, ParticleSystem ps)
        {
            ...
            for (var i = 0; i < numberOfClips; i++)
            {
                ...
                if (currentTime < currClipEnd) // Simulate part of a clip until the current time 
                {
                    em.enabled = true;
