...
{
    public class ComplexMoveState : CharacterState
    {
        ...
        [SerializeField]
        private ManualTimelinePlayer enterMovingPlayableAsset;
        ...
        private void SelectMoveState(Vector2 moveInput)
        {
            if (movementController.GetIsGrounded())
            {
                ...
                else if (currentSubState != MoveSubStates.Moving)
                {
                    SetSubState(MoveSubStates.EnterMoving);
                }
                else
                {
        ...
        private void SetSubState(MoveSubStates subState)
        {
            ...
            switch (subState)
            {
                case MoveSubStates.Idle:
                    ...
                    break;
                case MoveSubStates.EnterMoving:
                    currentPlayableAsset = enterMovingPlayableAsset;
                    break;
