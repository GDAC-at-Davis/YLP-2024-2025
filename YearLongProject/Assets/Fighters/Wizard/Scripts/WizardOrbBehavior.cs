...
namespace Fighters.Wizard.Scripts
{
    public class WizardOrbBehavior : Entity
    {
        ...
        [SerializeField]
        private Vector2 knockbackHitboxSize;
        ...
        private BoxArea knockbackHitboxArea;
        ...
        private void FixedUpdate()
        {
            ...
            {
                CheckForPlayerCollisions(Physics2D.BoxCastAll(transform.position, knockbackHitboxSize, 0, Vector2.zero,
        ...
        private void
            DoCollisions() // I couldn't figure out the weird collision interacions so I'm just doing this sorry
        {
            collisions.right = collisions.left = collisions.above = collisions.below = false;

            collisions.right = Physics2D.Raycast(transform.position + Vector3.right * col.bounds.size.x / 2,
                Vector3.right, rayDistance, physicsCollisionLayers);
            collisions.left = Physics2D.Raycast(transform.position + Vector3.left * col.bounds.size.x / 2, Vector3.left,
                rayDistance, physicsCollisionLayers);
            collisions.above = Physics2D.Raycast(transform.position + Vector3.up * col.bounds.size.y / 2, Vector3.up,
                rayDistance, physicsCollisionLayers);
            collisions.below = Physics2D.Raycast(transform.position + Vector3.down * col.bounds.size.y / 2,
                Vector3.down, rayDistance, physicsCollisionLayers);
        ...
        private void CheckForPlayerCollisions(RaycastHit2D[] hits)
        {
            ...
            {
                var entityHurtbox = hit.collider.GetComponent<EntityHurtbox>();
                if (entityHurtbox)
                {
                    var manager = (CharacterEntity)entityHurtbox.AttachedEntity;
                    if (manager == this.manager.Wizard)
                    {
                        return;
                    }

                    //if (manager.CurrentState.stateNameSO.StateType == State_Machine_Scripts.StateNameSO.StateTypes.HITSTUN)
                    if (manager.ActionManager.CurrentState.GetType() != typeof(HitstunState))
                    {
                        return;
                    }
